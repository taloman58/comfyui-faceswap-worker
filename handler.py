...
#    WEBSOCKET_RECONNECT_DELAY_S sets the sleep in seconds between attempts.
#
...
if os.environ.get("WEBSOCKET_TRACE", "false").lower() == "true":
    ...
    # protocol errors but can be noisy in production  therefore gated behind an env-var.
...
def _attempt_websocket_reconnect(ws_url, max_attempts, delay_s, initial_error):
    ...
    for attempt in range(max_attempts):
        ...
        if not srv_status["reachable"]:
            # If ComfyUI itself is down there is no point in retrying the websocket 
            # bail out immediately so the caller gets a clear "ComfyUI crashed" error.
            print(
                f"worker-comfyui - ComfyUI HTTP unreachable  aborting websocket reconnect: {srv_status.get('error', 'status '+str(srv_status.get('status_code')))}"
            )
        ...
        try:
            ...
            if error_details:
                detailed_message = f"{error_message}:\n" + "\n".join(
                    f" {detail}" for detail in error_details
                )
...
def handler(job):
    ...
    try:
        ...
        # -------------------------------------------------------------------
        # FRAME FLOOD PROTECTION: Video ciktisi varsa, ara frame'leri atla.
        # VHS_VideoCombine gibi node'lar birlesmis videoyu "gifs" key'inde verir.
        # SaveImage ise her frame'i ayri PNG olarak "images" key'inde verir.
        # Ikisi birlikte varsa 386 PNG + 1 MP4 = gereksiz yuk.
        # Cozum: video varsa, 10'dan fazla image ureten node'lari atla.
        # -------------------------------------------------------------------
        has_video_output = any("gifs" in node_out for node_out in outputs.values())
        FRAME_FLOOD_THRESHOLD = 10  # Bu sayidan fazla image = ara frame kabul edilir

        for node_id, node_output in outputs.items():
            if "images" in node_output:
                image_count = len(node_output["images"])

                # Video varsa ve bu node cok fazla image uretmisse, bunlar ara frame'dir
                if has_video_output and image_count > FRAME_FLOOD_THRESHOLD:
                    print(
                        f"worker-comfyui - Node {node_id}: {image_count} frame images SKIPPED "
                        f"(video output exists, threshold={FRAME_FLOOD_THRESHOLD})"
                    )
                    continue

                print(
                    f"worker-comfyui - Node {node_id} contains {image_count} image(s)"
                )
